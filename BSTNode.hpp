...
public:
  ...
  BSTNode<Data>* right;
  BSTNode<Data>* parent;
  Data const data;   // the const Data in curr node.
  ...
  /** Return the successor of curr BSTNode in a BST, or 0 if none.
   ** PRECONDITION: curr BSTNode is a node in a BST.
   ** POSTCONDITION:  the BST is unchanged.
   ** RETURNS: the BSTNode that is the successor of curr BSTNode,
...
BSTNode<Data>* BSTNode<Data>::successor()
{
  ...
  if(curr->parent != NULL){
    BSTNode* above = curr->parent;

    while(above != NULL){
      if(curr->data < above->data) {
        return above;
        //break;
      }
      else{ above=above->parent;}
    }
//    return curr;
  }
  return NULL;
